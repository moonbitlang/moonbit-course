...
<!-- 打开页面，为大家注意展示效果：运行hello.mbt、修改odd.mbt并添加运行代码，运行包，刷新环境 -->
...
<!-- 我们以腾讯云Coding为例，为大家展示使用方法。大家在创建项目之后，可以创建代码仓库。之后，可以点击云原生IDE进入开发环境。在左侧扩展面板中搜索Moonbit，即可找到语言插件。安装后，即可像浏览器一样使用。进阶应用可以导入模板。进入仓库后，会弹出安装建议。同时，命令行中也有命令可用。 -->
