...
- 哈希函数/散列函数
  - 将任意长度的数据映射到某一固定长度的数据
    - 一致性：同一个输入数据，输出的哈希值始终是唯一的
    - 输入敏感性：输入数据的微小变化会引起哈希值巨大的改变
    - 抗碰撞性：两个不同的输入具有相同哈希值的概率极低
  - 例如：MD5 算法可以把各种大小、各种格式的文件映射到 128 位的数据上；可以用于快速校验两个文件是否相同
- 可以定义如下的 `Hash` 特征，将数据映射到整数范围内
...
<!-- ssml
  ...
  一般来说，<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>函数具有如下几个性质，
  第一，一致性，对于同一个输入，输出的<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>值始终是唯一的；
  第二，输入敏感性，只要输入发生了一点微小的变化，输出都会发生不可预测的巨大改变；
  第三，抗碰撞性，碰撞也叫<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>冲突，
  指的是两个不同的输入产生具有相同的<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>值。合格的<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>函数具有抗碰撞性，
  也就是发生<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>冲突的概率极低。
  举个例子来说，大家可能听说过 MD5 算法，可以把各种大小、各种格式、无穷无尽的文件映射到短短的 128 位的数据上来。
  MD5 算法可以用于快速校验两个文件是否相同，因为只要稍有不同，MD5 算法的输出都会发生巨大差异。
  在下方的例子中，我们定义了 <lang xml:lang="en-US">Hash</lang> 特征，该特征声明了一个 <phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>函数，将数据映射到整型的范围内。
...
// 键 --哈希--> 哈希值 --取模--> 数组索引
let index = key.hash().abs() % array.length()
...
<!-- ssml
  ...
  <phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>值的范围是所有整数，因此，之后我们根据这个<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>值取绝对值、再取模 映射到数组索引上，
  基于这个索引我们可以进行取值、修改操作。
...
- 添加数据时：计算哈希值及对应索引，将数据添加到索引对应的数据结构之中
...
<!-- ssml
  首先我们从较为简单的直接寻址开始。
  简单来说，就是定义一个数据结构的数组。
  当数据将要被添加进来，则计算它们的<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>值及对应索引，
  最后将数据添加到索引对应的数据结构之中。
  <break time="500ms" />
-->
...
struct Bucket[V] {
  mut next : Option[(V, Bucket[V])]  // None = 空列表, Some = (头元素, 剩余)
}
...
fn[K : Hash + Eq, V] HT_bucket::put(self : HT_bucket[K, V], key : K, value : V) -> Unit {
  let index = key.hash().abs() % self.length // 计算对应索引
  let mut bucket = self.values[index]        // 找到对应列表
  while bucket.next is Some((entry, rest)) {  // 遍历列表查找键
    if entry.key == key {  // 找到键，更新值
      entry.value = value
      break
    } else {
      bucket = rest        // 继续查找
    ...
    bucket.next = Some(({ key, value }, { next: None })) // 未找到，添加新键值对
...
<!-- ssml
  ...
  这里展示了使用可变数据加上 while 循环的操作。
  当查找到对应键，更新数据，然后进行 break 操作，跳出循环。
  否则，我们将 bucket 替换为剩余的队列，确保循环过程中数据都在变为更小的结构，继续遍历。
  如果到队尾了都没有找到，那么进入 else 块，在队尾添加一对新的数据。
...
fn[K : Hash + Eq, V] HT_bucket::remove(self : HT_bucket[K, V], key : K) -> Unit {
  ...
  // 遍历列表查找键
  while bucket.next is Some((entry, rest)) {
    if entry.key == key {
      // 找到键，删除（通过修改指针）
      bucket.next = rest.next
      self.size = self.size - 1
      break
    } else {
      bucket = rest  // 继续查找
...
fn[K : Hash + Eq, V] HT_open::find_slot(self : HT_open[K, V], key : K) -> Int {
  let mut i = key.hash().abs() % self.length

  while self.occupied[i] {
    if self.values[i].key == key {
      return i  // 找到键
    } else {
      i = (i + 1) % self.length // 线性探查
    }
  }
  return i  // 找到空位
}
...
struct HT_open_tombstone[K, V] {
  mut values : Array[Status[K, V]] // 改为使用 Status 而非 Entry 和 Bool
  mut length : Int
  mut size : Int
}
```
...
<!-- ssml
  我们定义一个新的状态的枚举类型，包括"空"、"被占据"以及"被删除"，并且将<phoneme alphabet="sapi" ph="ha 1 xi 1">哈希</phoneme>表内的字段也进行对应的修改。
  其中，被占据 的状态是一个键值对。
...
fn[K : Hash + Eq, V] HT_open_tombstone::find_slot(
  ...
  let mut slot = None // 已经找到的空位
  for index = key.hash().abs() % self.length; ; index = (index + 1) %
        self.length {
    match self.values[index] {
      Empty => return slot.unwrap_or(index) // 找到第一个空位或返回更早的空位
      Deleted => if slot is None { slot = Some(index) } // 更新空位索引
      Occupied(k, _v) => if k == key { return index } // 键已经存在
    }
  }
}
```
...
<!-- ssml
  ...
  在第 5 行，用一个可变变量 slot 表示空位索引。
  接下来使用一个循环来遍历所有的索引。
  当找到 <lang xml:lang="en-US">Empty</lang> 时，我们停止查找，并返回第一个空位。
  此时，如果之前已经找到空位，就返回已经找到的空位，否则说明这是第一个空位，返回这个空位即可。
  我们使用 Option 类型的 unwrap or 方法，当 slot 是一个 Some 时，直接返回 Some 包裹的值；
  否则，返回 unwrap or 的参数。
  当遇到被删除元素留下的墓碑 <lang xml:lang="en-US">Deleted</lang> 时，如果这是第一次遇到，我们将空位索引 更新<phoneme alphabet="sapi" ph="wei 2">为</phoneme>墓碑的索引。
  然后，我们也需要一直向后找，直到找到空位或者对应的键。
  在第11行，匹配了被占据的键值对，此时如果刚好匹配到我们在找的键，返回索引即可，否则继续查找。
  <break time="500ms" />
-->
...
```moonbit
fn[K : Hash + Eq, V] HT_open_tombstone::remove(
  self : HT_open_tombstone[K, V],
  key : K,
) -> Unit {
  let index = self.find_slot(key)
  if self.values[index] is Occupied(_) {
