...
- `(Int) -> ((Int) -> Int)` 接受一个整数并返回一个函数，该函数接受一个整数并返回一个整数
  可省略箭头：`(Int) -> (Int) -> Int`
...
```moonbit 
fn concat(l1 : List[Int], l2 : List[Int]) -> List[Int] {
  match l1 {
    Nil => l2
...
$\mapsto$ `Cons(1, concat(Cons(2, Nil), Cons(3, Nil)))`
$\mapsto$ `Cons(1, Cons(2, concat(Nil, Cons(3, Nil))))`
$\mapsto$ `Cons(1, Cons(2, Cons(3, Nil)))`
...
- 命题：对于任意列表 `l1` 和 `l2`，拼接后的列表 `concat(l1, l2)` 的长度为 `l1` 的长度与 `l2` 的长度之和。

- 证明：对 `l1` 分类讨论：
  - 若 `l1` 为空（`Nil`），则拼接后的列表的长度为 
  `length(concat(Nil, l2))` 
  $\mapsto$ `length(Cons(Nil, l2))`（替换 `concat` 的定义）
  $\mapsto$ `length(l2)`（替换 `length` 的定义）
  该结果等于 `length(l1) + length(l2)`，因为 `length(l1)` 为 `0`
...
- 证明：
  - 若 `l1` 非空（`Cons(hd, tl)`），则拼接后列表长度为 
  `length(concat(l1, l2))` 
  $\mapsto$ `length(concat(Cons(hd, tl), l2))`（模式匹配）
  $\mapsto$ `length(Cons(hd, concat(tl, l2)))`（由 `concat` 的定义）
  $\mapsto$ `1 + length(concat(tl, l2))`（由 `length` 的定义）
  $\mapsto$ `1 + (length(tl) + length(l2))`（归纳假设）
  $\mapsto$ `(1 + length(tl)) + length(l2)`（加法结合律）
  $\mapsto$ `length(l1) + length(l2)`（在 `length(l1)` 替换 `length` 的定义）
  - 根据数学归纳法，命题成立
...
不同的斐波那契数列的计算方式带来的性能差别（`num` > 40）

```moonbit skip
fn fib1(num : Int) -> Int {
  if num == 1 || num == 2 { 1 } else { fib1(num - 1) + fib1(num - 2) }
}
fn fib2(num : Int) -> Int {
  fn aux(n, acc1, acc2) {
    match n {
...
<!-- fn fib2(num : Int) -> Int {
  loop num, 0, 0 {
    0, acc1, acc2 => break acc1 
    1, acc1, acc2 => break acc2 
    n, acc1, acc2 => continue n - 1, acc2, acc1 + acc2 
  }
} -->
...
- 以求解斐波那契数列为例，我们假设的 `Cache` 应有如下接口：

```moonbit no-check
fn empty() -> Cache                                      // 创建数据结构
fn put(cache : Cache, num : Int, value : Int64) -> Cache // 存储数据，只执行一次
fn get(cache : Cache, num : Int) -> Option[Int64]        // 提取数据
```

- 符合条件的数据结构有很多，我们的样例代码以 `@immut/hashmap.T[Int, Int64]` 为例（后文记作 `Map[Int, Int64]`）
...
fn fib1(num : Int) -> Int64 {
  ...
  let map = put(put(empty(), 1, 1L), 2, 1L)
  aux(num, map).0
}
```
...
fn fib1_mut(num : Int) -> Int64 {
  let mut map = put(put(empty(), 1, 1L), 2, 1L) // 通过let mut声明可变变量
...
fn fib2(num : Int) -> Int64 {
  ...
  let map = put(put(empty(), 0, 0L), 1, 1L)
  aux(1, map)
}
```
...
- 我们每次只需保存当前项的前两个值，因此我们可以直接通过递归参数传递
